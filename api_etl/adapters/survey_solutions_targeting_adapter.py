...
#                 # ------------------- Group Code -------------------
#         ik = self._get(
#             record, "interview__key", "Interview__Key", "interview_key"
...
#         # IMPORTANT: also forbid "json_ext" itself to prevent double-nesting if it appears upstream
#         forbidden = {
#             "location_code",
#             "location_name",
#             "group_code",
...
#             # never store service metadata (e.g. "_source") in raw
#             if key.startswith("_"):
#                 continue
...
#             # Keep raw values simple; avoid dict/list that break workflow
#             if isinstance(v, (str, int, float, bool)) or v is None:
#                 json_ext["raw"][key] = v
#             else:
#                 json_ext["raw"][key] = str(v)

#         # ---- Promote keys needed for filtering to TOP-LEVEL Json_ext ----
#         record_type_val = self._get(record, "record_type", "ss_record_type", "RecordType")
#         consent_val = self._get(record, "consent_res", "Consent_Res", "CONSENT_RES")
#         wave_val = self._get(record, "pssn_wave", "PSSN_WAVE", "wave")

#         if record_type_val is not None and str(record_type_val).strip() != "":
#             json_ext["record_type"] = str(record_type_val).strip()
#             # keep namespaced copy in raw too (optional, but nice for debugging)
#             json_ext["raw"]["ss_record_type"] = str(record_type_val).strip()

#         if consent_val is not None and str(consent_val).strip() != "":
#             json_ext["consent_res"] = str(consent_val).strip()
#             json_ext["raw"]["ss_consent_res"] = str(consent_val).strip()

#         if wave_val is not None and str(wave_val).strip() != "":
#             json_ext["pssn_wave"] = str(wave_val).strip()
#             json_ext["raw"]["ss_pssn_wave"] = str(wave_val).strip()
