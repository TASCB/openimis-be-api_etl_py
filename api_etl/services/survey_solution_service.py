...
class SurveySolutionService(_BaseService):
    ...
    def _is_person_row(self, row: Dict[str, Any]) -> bool:
        if not isinstance(row, dict):
            return False
    ...
    def _merge_household_roster(self, all_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for r in all_rows:
            ...
            if not ik:
                LOG.debug("Row without Interview Key skipped: keys=%s", list(r.keys())[:8])
                continue
        ...
        LOG.info("Roster merge: %s interviews → %s merged person rows", len(bucket), len(merged))
        return merged
    ...
    def build_preview_payload(self, records, source_name=None):
        ...
        from individual.apps import IndividualConfig  # safe import here
    ...
    def run(
        ...
        **source_overrides,
        ...
        # Effective tab filter + include_meta default from config
        effective_tab = tab_filter or tab_name_contains or cfg.get("export_tab_name_contains") or None
